# script general para crear indicadores de SCL
  ...
  if (tipo == "censos") {
    ...
    variables_censos <- readxl::read_xlsx("Inputs/D.7.1.3 Diccionario variables censosvVivienda.xlsx")
...
source("functionsVivienda.R")

##### Use parallel programming -----

# read the indicators definitions in the csv
indicator_definitions <- read.csv("Inputs/idefvVivienda.csv") %>% filter(gdiCensus=="Test")
...
temp = list.files(path = paste(getwd(),"/Outputs/",sep=""),pattern = paste("indicadores_censos_hogares_vivienda_", pais,"_",anio,".+.csv",sep = ""))
