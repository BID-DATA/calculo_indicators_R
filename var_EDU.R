  ...
  data_filt <- data_filt %>% 
    mutate(age_25_mas = dplyr::if_else(edad_ci>=25, 1, 0),
           ...
           asis_net_prim_c = dplyr::case_when((pais_c=="COL"|pais_c=="BRA") & (aedu_ci>=0 & aedu_ci<5) & (asiste_ci==1 & age_prim_c==1)~ 1,
                                       ...
                                       TRUE ~ NA_real_),
           asis_prim_c = dplyr::case_when((pais_c=="COL"|pais_c=="BRA") & (aedu_ci>=0 & aedu_ci<5) & (asiste_ci==1 & edad_ci>=6)~ 1,
                                   ...
                                   TRUE ~ NA_real_),
           age_prim_sobre = dplyr::case_when( edad_ci<6 | is.na(aedu_ci) ~ NA_real_,
           ...
           asis_net_seco_c = dplyr::case_when((pais_c=="CRI"|pais_c=="GTM"|pais_c=="GUY"|pais_c=="JAM"|pais_c=="NIC"|pais_c=="PER"|pais_c=="VEN") & (aedu_ci>=6 & aedu_ci<11) & asiste_ci==1 & age_seco_c==1~ 1,
                                       ...
                                       TRUE ~ NA_real_),        
           asis_seco_c = dplyr::case_when((pais_c=="CRI"|pais_c=="GTM"|pais_c=="GUY"|pais_c=="JAM"|pais_c=="NIC"|pais_c=="PER"|pais_c=="VEN") &  (aedu_ci>=6 & aedu_ci<11) & asiste_ci==1  & edad_ci>=6~ 1,
           ...
           asis_net_tert_c = dplyr::case_when((pais_c=="COL"|pais_c=="CRI"|pais_c=="GTM"|pais_c=="GUY"|pais_c=="JAM"|pais_c=="NIC"|pais_c=="PER"|pais_c=="VEN") & aedu_ci>11 & asiste_ci==1 & age_tert_c==1 ~ 1,
                                       ...
                                       TRUE ~NA_real_),
           asis_tert_c = dplyr::case_when((pais_c=="COL"|pais_c=="CRI"|pais_c=="GTM"|pais_c=="GUY"|pais_c=="JAM"|pais_c=="NIC"|pais_c=="PER"|pais_c=="VEN") & aedu_ci>11 & asiste_ci==1 & edad_ci>=6 ~ 1,
           ...
           grupo_etario = dplyr::case_when(edad_ci>=4 & edad_ci<=5~ "age_4_5",
                                    ...
                                    TRUE ~NA_character_),
           anos_edu = dplyr::case_when(aedu_ci==0 & (!is.na(aedu_ci) | !is.na(edad_ci))  ~ "anos_0",
           ...
           age_15_29_edu = dplyr::if_else(edad_ci>=15 & edad_ci<=29 & !is.na(asiste_ci), 1, 0),
           age_18_24_edu = dplyr::if_else(edad_ci>=18 & edad_ci<=24 & !is.na(asiste_ci), 1, 0),
           age_18_23_edu = dplyr::if_else(edad_ci>=18 & edad_ci<=23 & !is.na(asiste_ci), 1, 0))
...
if (tipo == "encuestas") {
  ...
  data_filt <- data_filt %>% 
    mutate(age_25_mas = dplyr::if_else(edad_ci>=25, 1, 0),
           ...
           leavers = dplyr::if_else((edupi_ci==1 | edupc_ci==1 | edus1i_ci==1 | edus1c_ci==1) & (asiste_ci == 0),1,0),
           leavers = dplyr::if_else(is.na(edupc_ci),NA,leavers),
           tprimaria = dplyr::if_else((edupc_ci ==1 | edusi_ci==1 | edusc_ci==1 | eduui_ci==1 | eduuc_ci==1),1,0),
           tprimaria = dplyr::if_else(is.na(edupc_ci),NA,tprimaria),
           tsecundaria  = dplyr::if_else((edusc_ci ==1 | eduui_ci==1 | eduuc_ci==1),1,0),
           tsecundaria = dplyr::if_else(is.na(edupc_ci),NA,tsecundaria),
           tsuperior = dplyr::if_else((eduuc_ci ==1),1,0),
           tsuperior = dplyr::if_else(is.na(edupc_ci),NA,tsuperior),
           t_cond_primaria   = dplyr::if_else((tprimaria==1   & age_term_p_c==1),1,0),
           t_cond_secundaria = dplyr::if_else((tsecundaria==1 & age_term_s_c==1),1,0),
           # 5 Desagregación por niveles
           nivel_ed_ = dplyr::case_when(eduno_ci==1 | edupi_ci==1~ 0,
                                 edupc_ci==1 | edusi_ci==1~ 1,
                                 edusc_ci==1 | eduui_ci==1~ 2,
                                 eduuc_ci==1~ 3),
           nivel_ed_1 = dplyr::case_when(eduno_ci==1~ 0,
                                  edupi_ci==1~ 1,
                                  edupc_ci==1~ 2,
                                  edusi_ci==1~ 3,
                                  edusc_ci==1~ 4,
