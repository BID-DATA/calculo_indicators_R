  ...
  data_filt <- data_filt %>% 
    mutate(age_25_mas = ifelse(edad_ci>=25, 1, 0), 
           ...
           asis_seco_c = case_when((pais_c=="CRI"|pais_c=="GTM"|pais_c=="GUY"|pais_c=="JAM"|pais_c=="NIC"|pais_c=="PER"|pais_c=="VEN") &  (aedu_ci>=6 & aedu_ci<11) & asiste_ci==1  & edad_ci>=6~ 1,
                                   ...
                                   TRUE ~ NA_real_), 
           age_term_s_c = case_when((pais_c=="COL"|pais_c=="CRI"|pais_c=="GTM"|pais_c=="GUY"|pais_c=="JAM"|pais_c=="NIC"|pais_c=="PER"|pais_c=="VEN")&(edad_ci>=19&edad_ci<=21)~ 1, ## DUDA PARA DAVID: AQUI LOS MISSING DE EDAD CI SIGEN COMO NA, SINO DEBEMOS AGREGARLO EN EL CODIGO NO QUEREMOS QUE LOS NA SE TRANSFORMEN EN 0
           ...
           grupo_etario = case_when(edad_ci>=4 & edad_ci<=5 & (!is.na(edad_ci))~ "age_4_5",
                                    edad_ci>=6 & edad_ci<=11 & (!is.na(edad_ci))~ "age_6_11",
                                    edad_ci>=12 & edad_ci<=14 & (!is.na(edad_ci))~ "age_12_14",
                                    edad_ci>=15 & edad_ci<=17 & (!is.na(edad_ci))~ "age_15_17",
                                    edad_ci>=18 & edad_ci<=24 & (!is.na(edad_ci))~ "age_18_24",
                                    TRUE ~NA_character_),
           anos_edu = case_when(aedu_ci==0 & (!is.na(aedu_ci))  ~ "anos_0",
                                aedu_ci>=1 & aedu_ci <=5 & (!is.na(aedu_ci)) ~ "anos_1_5", 
                                aedu_ci>=6 & aedu_ci <=6 & (!is.na(aedu_ci)) ~ "anos_6", 
                                aedu_ci>=7 & aedu_ci <=11 & (!is.na(aedu_ci))  ~ "anos_7_11", 
                                aedu_ci>=12 & aedu_ci <=12 & (!is.na(aedu_ci)) ~ "anos_12",
                                aedu_ci>=13 & (!is.na(aedu_ci))  ~ "anos_13_mas",
                                TRUE ~NA_character_),
