...
scl_pct <- function(.data, .nombre, .condicion1, .condicion2, .group_vars) {
  ...
  if('disability_ch' %in% .group_vars){
    data_aux <- data_aux %>% rename(disability = disability_ch)
  }
...
scl_pctv2 <- function(.data, .nombre, .condicion1, .condicion2, .group_vars) {
  ...
  .condicion1 <- rlang::parse_expr(.condicion1)
  .condicion2 <- rlang::parse_expr(.condicion2)
  vector2 <- c("ethnicity","disability","area")  
  if (!is.null(.group_vars)) {
    data_aux <- .data %>%
      dplyr::group_by_at(.group_vars) %>%
      dplyr::summarise(
...
calculate_indicators <- function(data, indicator_definitions) {
  ...
  if (tipo=="censos"){
    ...
    write.csv(res, paste("Outputs/indicadores_censos_hogares_demographic_", pais,"_",anio,i,".csv",sep = ""), row.names=FALSE)
    rm(res_list)
    gc()
    res_list <- list()
  }
